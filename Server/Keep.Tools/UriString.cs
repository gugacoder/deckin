  ...
  public class UriString
  {
        ...
        if (!string.IsNullOrEmpty(part.Path))
        {
          ...
          else
          {
            ...
            while (part.Path.StartsWith(".."))
            {
              ...
              target.Path = string.Join("/", targetTokens.Take(
    ...
    public UriString SetProtocol(string value)
    {
      ...
      if (!Regex.IsMatch(value, @"^[\w\d]+(://)?$"))
        throw new FormatException("Formato inválido para um protocolo. " +
          @"Exemplo de formato suportado: ""ftp"" e ""http://""");
    ...
    public UriString SetHost(string value)
    {
      ...
      if (!Regex.IsMatch(value, @"^[\w\d-._]+(:\d+)?$"))
        throw new FormatException("Formato inválido para um nome de host. " +
          @"Exemplo de formato suportado: ""host.com"" ""host.com:8080""");
    ...
    public UriString SetPath(string value)
    {
      ...
      if (!Regex.IsMatch(value, @"^$|^[^?]+"))
        throw new FormatException("Formato inválido para um caminho de URI. " +
          @"Exemplo de formato suportado: ""usuarios/123""  ""/usuarios/123""");
    ...
    public UriString Combine(string uriPart, string otherUriPart,
      params string[] otherUriParts)
    {
      var terms =
        uriPart.AsSingle().Append(otherUriPart).Concat(otherUriParts).ToArray();
